...
class CharSearchSkillLayoutView : LinearLayout {
    ...
    fun setImageText(skill: ArmorySkill,tooltip:Tooltip){
        Glide.with(this)
            .load(skill.icon)
            .into(skillImage)
        imageUrl = skill.icon
        ...
        skillType = (tooltip.elements.get("Element_001")?.value as CommonSkillTitleData).name
        skillStance = " | "+(tooltip.elements.get("Element_001")?.value as CommonSkillTitleData).level.replace("[","").replace("]","")
        skillCool = (tooltip.elements.get("Element_001")?.value as CommonSkillTitleData).leftText
