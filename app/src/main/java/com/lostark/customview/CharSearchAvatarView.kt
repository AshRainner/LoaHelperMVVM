...
class CharSearchAvatarView : LinearLayout {
    ...
    fun setImageText(avatar: ArmoryAvatar, tooltip: Tooltip) {
        ...
        if (itemPartKeyList.isNotEmpty()) {
            defaultEffect =
                (tooltip.elements.get(itemPartKeyList.get(0))?.value as ItemPartData).element1
        }
        ...
        if (symbolStringList?.isNotEmpty() ?: false) {
            symbolString =
                (tooltip.elements.get(symbolStringList?.get(0))?.value as SymbolStringData).contentStr
        }
